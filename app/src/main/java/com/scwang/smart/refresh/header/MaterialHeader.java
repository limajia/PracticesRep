...
//        mProgress.setAlpha(255);
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        circleView.measure(MeasureSpec.makeMeasureSpec(mCircleDiameter, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(mCircleDiameter, MeasureSpec.EXACTLY));
    }
    ...
    public void onMoving(boolean isDragging, float percent, int offset, int height, int maxDragHeight) {
        ...
        if (isDragging || (!mProgress.isRunning() && !mFinished)) {
            ...
            if (mState != RefreshState.Refreshing) {
                ...
                float tensionSlingshotPercent = Math.max(0, Math.min(extraOS, (float) height * 2) / (float) height);
